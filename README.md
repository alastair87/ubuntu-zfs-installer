...
If `--user-password-hash` is not provided, the installer prompts for the
initial user's password while configuring the target system. For unattended
installs, provide an encrypted hash such as one generated by `openssl passwd -6`.

Use `--swap-size 0` to omit the swap partition and all swap configuration.
